...
(function() {
  var Timetress, date, easterDate, easterDates, easterHash, i, len, parts;
  ...
  for (i = 0, len = easterDates.length; i < len; i++) {
    date = easterDates[i];
    parts = date.split("-");
  ...
  Timetress = (function() {
    ...
    Timetress.nextHoliday = function(holiday, givenDate) {
      ...
      theDay = this[holiday](givenDate.getUTCFullYear());
      if (givenDate.getTime() > theDay.getTime()) {
        theDay = this[holiday](givenDate.getUTCFullYear() + 1);
      }
      return theDay;
    };
